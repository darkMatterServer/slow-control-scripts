...
def run_commands():
    try:
        ...
        subprocess.Popen((['python3', 'MFC-serial-grep-data-sender.py'], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

        # Run the second command and suppress output
        subprocess.Popen((['python3', 'pressure-serial-grep-data-sender.py'], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
